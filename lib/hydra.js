...
function configure_hydra (app, config) {
  ...
  function initialize_hydra (req, res, next) {
    ...
    hydraAdmin
      ...
      .then(function (login_challenge) {
        console.log("HYDRA LOGIN REQUEST RESPONSE", login_challenge);
        ...
        hydraClient(login_challenge, acceptLoginRequest)
          .then(function login_response (login_response) {
            res.redirect(String(login_response.data.redirect));
  ...
  function render_consent (req, res, next) {
    ...
    hydraAdmin
      ...
      .then(function (consent_request) {
				console.log("CONSENT REQUEST", consent_request);
        ...
        res.locals = consent = consent_request;
        res.render('hydra/consent', {
          ...
          action: urljoin(app.config.oauth_base, '/connect/consent')
        });
      })
  }
  ...
  function process_consent_attempt (req, res, next) {
    ...
    hydraAdmin
      .getConsentRequest(challenge)
      .then(function (consent_request) {
        console.log("GRANTING CONSENT FOR USER", req.user, oauth_session, consent_request);
        return hydraAdmin
          .acceptConsentRequest(challenge, {
