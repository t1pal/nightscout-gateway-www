...
function create_middleware (app, config) {
  ...
  function debug_views_default (req, res, next) {
    res.locals.debugViews = config.debugViews;
    next( );
  }

  app.locals.debugViews = config.debugViews;
  var wares = {
...
function defaultRoutes (app, config) {
  ...
  const deactivationRoute = require('../deactivation')(app, config);
  ...
  router.post('/deactivate', kratosSession.handlers.ensureLoggedIn, set_links, deactivationRoute( ));
  ...
  const groupsRoutes = require('../groups')(app, config);
  router.use('/groups/', kratosSession.handlers.ensureLoggedIn, set_links, groupsRoutes( ));

  router.get('/', kratosSession.handlers.checkLoggedIn, set_links, index);
