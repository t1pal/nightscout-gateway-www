...
function defaultRoutes (app, config) {
  ...
  router.get('/auth/registration', registrationRoute( ));
  router.get('/auth/login', loginRoute( ));
  router.get('/auth/logout', logoutRoute( ));
  router.get('/error', errorRoute( ));

  router.get('/verify', verificationRoute( ));
  router.get('/recovery', recoveryRoute( ));


  router.get('/dashboard', kratosSession.handlers.ensureLoggedIn, dashboardRoute( ));

  router.get('/settings', kratosSession.handlers.ensureLoggedIn, settingsRoute( ));

  const hydraRoutes = require('../hydra')(app, config);

  router.use('/auth/hydra/', kratosSession.handlers.ensureLoggedIn, hydraRoutes( ));
  ...
  router.use('/invitations/', kratosSession.handlers.ensureLoggedIn, invitationRoutes( ));
  ...
  router.use('/portal/', kratosSession.handlers.ensureLoggedIn, portalRoutes( ));
